...
def gen_cv_samples(X_train_df: pd.DataFrame, y_train_df: pd.DataFrame, n_cv_folds: int) -> tuple:
    ...
    try:
        ...
        kf_indices = [(train, test) for train, test in
                      kf.split(X_train, y_train)]  # making list of indices to be used for folds based on KFold object
...
def comparison(id: int, which_regressors: dict, metric_list: list, styledict: dict, n_visualized_bp: int, n_visualized_tb: int, test_set_size: float,
               n_cv_folds: int, score_method: str, datapath: str) -> str:
    """
    ...
    Args:
        ...
        n_visualized_bp (int) - the top scoring 'n' regressors in cross-validation to be included in boxplot visualizations. The value -1 will include all regressors (Default: -1)
        n_visualized_tb (int) - the top scoring 'n' regressors over the test set to be included in final table. The value -1 will include all regressors (Default: -1)
        test_set_size (float) - a number between 0 and 1 that indicates the proportion of data to be allocated to the test set (Default: 0.2)
        n_cv_folds (int) - the number of folds for k-fold cross validation training (Default: 10)
    ...
    """
    print(
        f'id: {id}, datapath: {datapath}, which_regressors: {which_regressors}, metric_list: {metric_list}, score_method: {score_method}, n_cv_folds: {n_cv_folds}, test_set_size: {test_set_size}')
    try:
        ...
        args_lst = [(regs[i // n_cv_folds], reg_names[i // n_cv_folds], metric_list, metric_help, cv_X_train[i % n_cv_folds],
                     cv_y_train[i % n_cv_folds], cv_X_test[i % n_cv_folds], cv_y_test[i % n_cv_folds]) for i in range(len(regs) * n_cv_folds)]
        try:
...
def run(reg: object, reg_name: str, metric_list: list, metric_help: dict, train_attrib: np.ndarray, train_labels: np.ndarray, test_attrib: np.ndarray,
    ...
    try:
        ...
        print(f'exception in run: {str(e)}')
...
def test_best(fin_org_results: dict, metric_list: list, test_attrib: pd.DataFrame, test_labels: pd.DataFrame, metric_help: dict,
              n_vizualized_tb: int) -> plt.figure:
    """
...
def email(recipient_list: list, message: str) -> None:
    ...
    Returns:
        None
    """
    print('sorry.. not sending email for now')
    # try:
