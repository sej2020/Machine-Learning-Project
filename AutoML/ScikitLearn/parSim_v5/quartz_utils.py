...
def comparison(datapath, n_regressors, metric_list, n_vizualized, metric_help, n_workers, score_method='neg_mean_squared_errror') -> None:
    """
    ...
    # training each regressor in CV --- serial#######################################
        ...
        stop = perf_counter()
        elapsed = stop - start
        print(f"Time to execute regression: {elapsed:.2f}s")
