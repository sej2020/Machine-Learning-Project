...
def comparison(datapath, n_regressors, metric_list, n_vizualized, metric_help, score_method='neg_mean_squared_errror') -> None:
    ...
    for k in range(len(figs)):
        figs[k].savefig(f'fig_{k}.png', bbox_inches='tight', dpi=600.0)
    return f'time: {stop-start}, results length: {len(results)}'
...
def test_best(results, metric_list, test_attrib, test_labels, metric_help):
    """
    ...
    rows = []
    output = []
    for i in range(0,len(results), 10): # -> 0, 10, 20, ...
        rows.append(list(results[i].keys())[0])
        examine = results[i:i+10] # -> 10 regressor dict.

        scores = []
        models = []
        for j in examine:
            scores.append(list(j.values())[0][0])
            models.append(list(j.values())[0][-1])
        ...
        for m in metric_list:
            ...
            single_reg_output.append(round(calculated if m != "neg_root_mean_squared_error" else calculated**.5,4))
...
def test_best2(results, reg_names, metric_list, test_attrib, test_labels, metric_help):
    """
    This function will take the best performing model on each regressor type generated by cross-validation training and 
    apply it to the set of test data. The performance of the regs on the test instances will be displayed on a table and
    saved to the user's CPU as a png file. The regs will be sorted in descending order by performance on specified metrics.
