...
def main(in_path, out_path):
    df = pd.DataFrame(columns=["location_x", "location_y", "rotation", "major_axis_length", "minor_axis_length", "MAE"])
            ...
            for metric, regr_dict in metric_dict.items():
                for regr, ax_dict in regr_dict.items():
                    for axes, error in ax_dict.items():
                        if metric == "MAE" and regr == "sklearn-svddc":
...
if __name__ == "__main__":
    ...
    in_path = "BetaDataExper/HyperEllipsoid/big_hyp_sim.yaml"
    out_path = "BetaDataExper/HyperEllipsoid/pred_error.csv"
